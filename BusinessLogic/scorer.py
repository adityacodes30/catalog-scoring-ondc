...
def main():
    ...
    def callback(ch, method, properties, body):
        ...
        \n Overall catalog score is {round(ts,2)*100} \n Data is as follows: \n {data}"""
        send(email, embody)
        ...
        channelToNotifQ.basic_publish(exchange='', routing_key='notificationQueue', body=data )
...
def processloop(json_data):
    ...
    for item in arr:
        ...
        score = topsis(name, long_desc, short_desc, image, symbol,labels)
        item["descriptor"]["score"] = score
        t_score = t_score + score

        print("Score:---------------------------->", item["descriptor"]["score"])
        
    return [json.dumps(json_data) , t_score/num]
